...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, allocation, onAdjust, canInvest }) => {
  ...
  return (
    <div className={`group relative flex flex-col bg-white rounded-xl border transition-all duration-200 ${isInvested ? 'border-concept-200 shadow-concept-50/50 shadow-md' : 'border-slate-200 hover:border-slate-300 hover:shadow-sm'}`}>
      ...
      <div className="h-40 w-full overflow-hidden rounded-t-xl relative bg-slate-100">
        ...
        {isInvested && (
          <div className="absolute bottom-0 left-0 right-0 bg-concept-500/90 backdrop-blur-sm py-1 px-3 flex justify-between items-center">
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="mb-1 flex items-center gap-2 text-xs font-medium text-slate-500 uppercase tracking-wide">
          <span className="text-concept-600">{project.theme}</span>
          <span>•</span>
          <span>{project.primaryArea}</span>
        </div>

        <h3 className="text-lg font-bold text-slate-900 mb-2 line-clamp-2 leading-tight group-hover:text-concept-700 transition-colors">
          {project.title}
        </h3>
          ...
          <div className="flex items-center justify-between text-sm pt-3 border-t border-slate-100">
             <div className="flex flex-col">
               ...
               <span className="font-semibold text-slate-700 flex items-center gap-1">
                 {project.totalHoursInvested} <TrendingUp size={12} className="text-concept-500"/>
               </span>
             </div>
      ...
      <div className="p-4 pt-0">
        <button 
          ...
          className={`w-full py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 
            ${isInvested 
              ? 'bg-concept-50 text-concept-700 border border-concept-200 hover:bg-concept-100' 
